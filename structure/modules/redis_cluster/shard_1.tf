# Atlas Redis Cluster Module - Shard 1 Resources
...
module "shard_1" {
  ...
  # per shard settings
  shard_id = "${var.shard_indices[0] + 1}"

  primary_subnet_id        = "${var.layer_data_subnet_ids[
    var.shard_indices[0] % length(var.layer_data_subnet_ids)
  ]}"

  # rotate replicas around availability zones per primary
  replica_subnet_ids       = ["${
    concat(
      slice(var.layer_data_subnet_ids, var.shard_indices[0] + 1 % length(var.layer_data_subnet_ids), length(var.layer_data_subnet_ids)),
      slice(var.layer_data_subnet_ids, 0, var.shard_indices[0] + 1 % length(var.layer_data_subnet_ids))
    )
  }"]
}
