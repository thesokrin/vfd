...
variable "additional_jenkinsmaster_cidr_blocks" {
  type = "list"
  description = <<-EOF
    Allow access to the Jenkins Master port to additional specified CIDR blocks.
  EOF
}
...
variable "additional_manager_cidr_blocks" {
  type = "list"
  description = <<-EOF
    Allow access to the Jenkins Master Manager port to additional specified CIDR blocks.
  EOF
}
...
variable "jenkinsmaster_instance_count" {
  default = 3
  description = <<-EOF
    Number of jenkinsmaster nodes to start.
  EOF
}

variable "jenkinsmaster_instance_type" {
  description = <<-EOF
    EC2 instance type for the jenkinsmaster instances.
  EOF
}
...
variable "jenkinsmaster_role_package" {
  description = <<-EOF
    The role package to install for Jenkins Master.
  EOF
}
