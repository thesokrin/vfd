# Atlas Staging Environment - Atlas Resources - Variables

variable "aws_account_id" {
  description = <<-EOF
    AWS Account ID to launch the environment in.

    This should always be staging for the Atlas Staging Environment.
  EOF
}
...
variable "env_name_fancy" {
  description = <<-EOF
    Fancy environment name for the Atlas Staging Environment.
  EOF
}
