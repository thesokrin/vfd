# Atlas PreProd - SonarQube resource - IAM Resources

# Storage IAM User, Group, and Policy
resource "aws_iam_user" "storage_manager" {
  name = "atlas_${var.env_name}_${replace(var.name, "-", "_")}_sonar_backend_storage_manager"
}

resource "aws_iam_group" "storage_group" {
  name = "atlas_${var.env_name}_${replace(var.name, "-", "_")}_sonar_backend_storage_group"
}
...
resource "aws_iam_policy" "storage" {
  name = "atlas_${var.env_name}_${replace(var.name, "-", "_")}_sonar_backend_storage_manager"
  description = "Allows read-write permissions to the sonar-backend storage bucket for ${var.name}.${var.zone}"
