# Atlas Development Environment - Mesos Services Workers - Variables
...
variable "layer_services_subnet_ids" {
  type = "list"
  description = <<-EOF
    Subnet ids for the three subnets in the services layer in the Atlas dev environment.
  EOF
}
...
variable "registry_elb_id" {
  description = <<-EOF
    ID of the Docker Registry Elastic Load Balancer.
  EOF
}
